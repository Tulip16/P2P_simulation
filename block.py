...
class P2P(object):
	...
	class Peer(object):
	    def __init__(self, p_id, p2p):
	    	...
	    	for txn_num in range(len(self.txn_times)):
	    		t_id = generate_t_id(self.p_id, txn_num)
	    		self.events.put((self.txn_times[txn_num], self.p2p.Event("gtxn",t_id)))
	    ...
	    def generate_txn(self, t_id, time):
	    	...
	    	print("pid %2d generated a transaction with TxnID: %2d at time: %2d"%(self.p_id, t_id, time))
	    ...
	    def receive_txn(self, t_id, time):
	    	print("pid %2d received transaction with TxnID: %2d at time: %2d"%(self.p_id, t_id, time))
