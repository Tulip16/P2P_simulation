...
class P2P(object):
    ...
    class block_tree:
        ...
        def insert(self, block):
            if(block.prev_b_id==self.block.b_id):
                self.children.append(block_tree(block, self.length+1))
            else:
                for c in self.children:
                    c.insert(block)
        '''def branch_length(self, block):
            int length = 0
            if(block.b_id==self.block.b_id):
                return length+1
            for c in self.children:
                length = c.branch_length(block)
                if(length>0):
                    return length+1'''
    ...
    class Peer(object):
        def __init__(self, p_id, status, p2p):
            ...
            self.block_tree = block_tree(current_block)
            self.txn_time = 0
            self.txn_num = 0
            self.coins = 1000
        ...
        def receive_block(self, b_id, time):
            ...
            if self.verify_block(b_id, time):
                #if self.block_tree==None:
                #    self.block_tree = block_tree(self.p2p.block_map[b_id])
                ...
                for peer in self.peers:
                    block_accounts.append(peer.coins)
                self.p2p.accounts[b_id] = block_accounts
                self.block_tree.insert(self.p2p.block_map[b_id])
                self.p2p.lengths[b_id] = self.block_tree.branch_length(self.p2p.block_map[b_id])
            else:
                print("INVALID!!")
