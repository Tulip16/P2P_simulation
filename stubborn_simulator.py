    ...
    class Peer(object):
            ...
            if not block.hidden:
                ...
                if self.verify_block(b_id, time): # if block is verified
                    if self.stubborn and not self.p2p.peers[block.miner-1].stubborn:
                        ...
                        # lead =  - self.block_node_map[self.longest[0]].length + self.block_node_map[self.stubborn_longest[0]].length         
                        self.receive_alert(-1, time, b_id)
        ...
        def handle_events(self, time):
            ...
            while math.ceil(event_time) == math.ceil(time):
                ...
                if(event.type == "ralt"):
                    self.receive_alert(event.sender, event_time, event.data)
                if(event.type == "salt"):
